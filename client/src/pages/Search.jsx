...
const mockDaycares = [
  {
    ...
    description: "A bright and cheerful environment where children can learn and grow through play and structured activities.",
  ...
  },
  {
    ...
    description: "Focused on developing curious minds through exploration and discovery-based learning.",
  ...
  },
  {
    ...
    description: "A home-based daycare offering personalized care in a warm, family environment.",
...
const Search = () => {
  ...
  const handleSearch = () => {
    ...
    const filtered = mockDaycares.filter(daycare => {
      ...
      const matchesPrice = daycare.price.length >= priceRange[0] && 
  ...
  return (
    <div style={{ padding: '24px' }}>
      ...
      <Paragraph>Search for daycares in your area and filter by your preferences</Paragraph>
      ...
      <List
        ...
        renderItem={item => (
          <List.Item
            key={item.id}
            actions={[
              <Space><ClockCircleOutlined /> {item.openingHours}</Space>,
              <Space><DollarOutlined /> {item.price}</Space>,
              <Space><TeamOutlined /> {item.ageGroups.join(", ")}</Space>
            ]}
            extra={
              <div style={{ textAlign: 'center' }}>
                <Rate disabled defaultValue={item.rating} />
                <div>{item.rating} / 5</div>
                <Button type="primary" style={{ marginTop: '16px' }}>View Details</Button>
              </div>
            }
          >
            <List.Item.Meta
              title={<Title level={4}>{item.name}</Title>}
              description={<Space><EnvironmentOutlined /> {item.address}</Space>}
            />
